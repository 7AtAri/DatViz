...
For our data visualization project, we were searching for a dataset that 
...
-   The dataset can be found at <https://www.kaggle.com/competitions/petfinder-adoption-prediction/data>.
    We also copied it to our google drive for a quick access: <https://drive.google.com/file/d/16tPNceHdB2oQOTaknP35rJQr_Bsa98Jq/view?usp=share_link>
...
To visualize the geographical location of the pets in our data, we decided to create an interactive map. Using a choropleth map on top of the map we visualize the distribution of the pets across Malaysia. Since we deal with skewed data which is also apparent in the map we use a log scale for the pet distributions. To get a better understanding of the locations we decided to include the population and GDP per capita in the map. With the interactivity of the map we also found a possibility to also plot the distribution of cats and dogs for each state using a pie chart.

To interact with the map one can click on the single states of Malaysia that are outlined and see their details such as the number of pets, population, GDP per capita and the distribution of cats and dogs in a pie chart. With the layer icon one can select the information that the choropleth map shows. The options are the number of pets, the population and the GDP per capita.

The shape data for the outline of the districts was retrieved by the International Steering Committee for Global Mapping [[1]](#1).
...
The sentiment scores found in the dataset as JSON files were transformed into a column and added to the tabular data. The score was calculated for the description column in the pet dataset, which means it analyzes the sentiment of the descriptions of the pets. To get more insight into how the sentiment score relates to the rest of the data, its distribution, and some bivariate statistics are plotted by pet type. The background gradient relates cold and warm colors to the range of the sentiment score, which goes from negative (-1) to positive (+1) sentiments. The necessary code for the background gradient was adapted from Stackoverflow [[2]](#2).
...
The pet data was clustered mainly by states and type (dog/cat) while cat and 
dog data were clustered by sterilized, vaccinated and dewormed mainly. The 
charts of the three clusters are interactive so that details of each point 
could be viewed.  

## Pets' Data Clustering

-   Pet data clustering is created using K-medoids clustering followed by t-SNE
...
-   cat data clustering is created using K-medoids clustering followed by t-SNE
...
-   dog data clustering is created using K-medoids clustering followed by t-SNE
-   It was done based on the following variables: StateId, BreedID1, BreedID2, Gender, MaturitySize, FurLength, Vaccinated, Dewormed, Sterilized, and Health
...
We ran the images that come with the pet data through a Resnet-101 Network and visualized the embeddings with T-SNE. It clustered the cats and dogs very well (left/red: dogs, right/blue: cats), but also sorted the animals by color (top: light fur, bottom: dark fur) and even captured some similarities that are common to both types in the middle, for example, the spot where pictures of animals in cages are displayed.
The necessary code [[3]](#3) was adapted and can be found in the neural network files folder. It was too big to be integrated as a python code junk.


## T-SNE 3D scatterplot without images as thumbnails
...
> <a id="id_1">[1]</a> International Steering Committee for Global Mapping. Malaysia. Jabatan Ukur dan Pemetaan. (2012). Districts, Malaysia, 2013. [Shapefile]. International Steering Committee for Global Mapping. Retrieved from <https://earthworks.stanford.edu/catalog/stanford-zd362bc5680>

> <a id="id_2">[2]</a> <https://stackoverflow.com/questions/30136725/plot-background-colour-in-gradient> 

> <a id="id_3">[3]</a> <https://learnopencv.com/t-sne-for-feature-visualization/> and
 <https://github.com/spmallick/learnopencv/tree/master/TSNE>
 
> <a id="id_2">[4]</a> <https://www.r-bloggers.com/2016/06/clustering-mixed-data-types-in-r/> 