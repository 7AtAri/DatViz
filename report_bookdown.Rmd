...
The shape data for the outline of the districts was retrieved by the International Steering Committee for Global Mapping [[2]](#2).
...
The Neural Network Embeddings Visualization has been done in Python and the various needed codefiles can be found in the "neural network files" Folder. Since the html output of our report file was already huge and took a very long time to knit, we decided not to try to embed that code into the report, but only use its outputs instead.
...
The necessary code [[5]](#5) was adapted and can be found in the neural network files folder. It was too big to be integrated as a Python code junk.
...
Using various methods, we tried to gain insights into the Pet Dataset. Having to deal with highly imbalanced data was one of the challenges, we had to face. To adequately display the data, we used for example log transformations for frequencies and heat map colorings, as well as boxplots for bivariate analysis. Our visualizations could be the basis for further analyzing the possible relationships between the sentiment score and pet's health, sentiment score and states, sentiment score and sterilization status, all with regards to the differences between the pet types. Another possible relationship might exist between the color of the dogs and the sentiment scores and between the Pet's States and their population, which seems to be reasonable. The clustering for each pet type attributes a lot to the categorical variables Vaccinated, Sterilized, Health and Dewormed. Visualizing the embeddings of the pet images in a Resnet-101 Neural Network, we learned how the network structures the images by type, animal color and as well as common settings like cages. Our visual exploration and analysis of the dataset could be used to help predict the adoption speed of the pets, which was the original purpose of the Kaggle challenge. Another possible further investigation would be to compare this data to pet data from other locations, since our data is only from Malaysia.
